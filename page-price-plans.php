<?php get_header();?>
<div class="pricing">
  <img class="itsm" src="/wp-content/uploads/2021/08/itsmframed.png">
  <img class="csm" src="/wp-content/uploads/2021/08/csmframed.png">
  <img class="cafm" src="/wp-content/uploads/2021/08/cafmframe.png">
  <img class="foi" src="/wp-content/uploads/2021/08/foiframed.png">
  <div class="pricing-container">
    <div class="intro">
      <h1>Affordable and feature packed</h1>
      <p><span>Available On-Demand or On-Premise<br>Prices based on annual billing</span></p>
    </div>
    <ul class="solutions">
      <li class="itsm">
        <div class="intro">
<<<<<<< HEAD
          <h3 class="header">IT Service <br>Management</h3>
          <div class="btn-cnt"><a class="more-info" href="/it-service-management/">Find out more</a></div>
=======
          <h3 class="header"><a href="/it-service-management/">IT Service <br>Management</a></h3>
>>>>>>> 5468ba44d75850f735d3d4c958322a2abc9f6b9d
          <div class="btn-cnt"><a class="sign-up" target="_blank" href="https://house-on-the-hill.com/mothership/register?A=QC&QC=QC000006">Start your free trial</a></div>
          <div class="price"><h3>£30</h3><p style="padding-left: 5px;"><span>/Per agent/month</span></p></div>
        </div>

        <div class="hl"></div>

        <div class="features">
          <p>Included:</p>
          <p><span>Incidents, Service Requests & Problem Management</span></p>
          <p><span>Configuration, SLA, Change & Release Management</span></p>
          <p><span>CMDB/Asset Management</span></p>
          <p><span>KPI Real-Time SLA Monitor</span></p>
          <p><span>Drag and Drop Form & Dashboard Designer</span></p>
          <p><span>Self-Service Portal (Unlimited End-Users)</span></p>
          <p><span>Seamless SSO and MS AD/Outlook365/Azure integration</span></p>
          <p><span>Feedback/Satisfaction Surveys</span></p>
        </div>
      </li>
      <li class="csm">
        <div class="intro">
<<<<<<< HEAD
          <h3 class="header">Customer Service Management</h3>
          <div class="btn-cnt"><a class="more-info" href="/customer-service-and-support/">Find out more</a></div>
=======
          <h3 class="header"><a href="/customer-service-and-support/">Customer Service Management</a></h3>
>>>>>>> 5468ba44d75850f735d3d4c958322a2abc9f6b9d
          <div class="btn-cnt"><a class="sign-up" target="_blank" href="https://house-on-the-hill.com/mothership/register?A=QC&QC=QC000004">Start your free trial</a> </div>
          <div class="price"><h3>£30</h3><p style="padding-left: 5px;"><span>/Per agent/month</span></p></div>
        </div>

        <div class="hl"></div>

        <div class="features">
          <p>Included:</p>
          <p><span>SLA Management</span></p>
          <p><span>Product & Inventory DB</span></p>
          <p><span>Org, Site & Contact DB</span></p>
          <p><span>Contract, POs & Invoices</span></p>
          <p><span>Self-Service Portal (Unlimited Customers)</span></p>
          <p><span>Job Sheets</span></p>
          <p><span>CRM</span></p>
        </div>
      </li>
      <li class="cafm">
        <div class="intro">
<<<<<<< HEAD
          <h3 class="header">Computer Aided Facilities Management</h3>
          <div class="btn-cnt"><a class="more-info" href="/facilities-management/">Find out more</a></div>
=======
          <h3 class="header"><a href="/facilities-management/">Computer Aided Facilities Management</a></h3>
>>>>>>> 5468ba44d75850f735d3d4c958322a2abc9f6b9d
          <div class="btn-cnt"><a class="sign-up" target="_blank" href="https://house-on-the-hill.com/mothership/register?A=QC&QC=QC000003">Start your free trial</a></div>
          <div class="price"><h3>£30</h3><p style="padding-left: 5px;"><span>/Per agent/month</span></p></div>
        </div>

        <div class="hl"></div>

        <div class="features">
          <p>Included:</p>
          <p><span>Reactive Work Requests</span></p>
          <p><span>Planned Preventative Maintenance</span></p>
          <p><span>Organisation, Site & Contacts DB</span></p>
          <p><span>Contract Management</span></p>
          <p><span>Quotes, POs, Invoices</span></p>
          <p><span>Compliance Forms</span></p>
          <p><span>Self-Service Portal (Unlimited End-Users)</span></p>
          <p><span>Bookings & Visitor Management</span></p>
        </div>
      </li>
      <li class="foi">
        <div class="intro">
<<<<<<< HEAD
          <h3 class="header">FOI, Case & Complaints Management</h3>
          <div class="btn-cnt"><a class="more-info" href="/foi-request-management/">Find out more</a></div>
=======
          <h3 class="header"><a href="/foi-request-management/">FOI, Case & Complaints Management</a></h3>
>>>>>>> 5468ba44d75850f735d3d4c958322a2abc9f6b9d
          <div class="btn-cnt"><a class="sign-up" target="_blank" href="https://house-on-the-hill.com/mothership/register?A=QC&QC=QC000001">Start your free trial</a></div>
          <div class="price"><h3>£30</h3><p style="padding-left: 5px;"><span>/Per agent/month</span></p></div>
        </div>

        <div class="hl"></div>

        <div class="features">
          <p>Included:</p>
          <p><span>FOI, EIR, DPA Management</span></p>
          <p><span>Complaints Management</span></p>
          <p><span>Contact Database</span></p>
          <p><span>Letter Generation</span></p>
          <p><span>Disclosure Log</span></p>
          <p><span>Info. Request Delegation</span></p>
          <p><span>Public Submission Form</span></p>
        </div>   
      </li>
    </ul>
  </div>
</div>

<div class="solution-table-container">
    <table class="solution-table-price">
      <tbody>
      <tr class="table-header">
        <th class="corefeatures">Core Features</th>
        <th class="featureprice">Feature Price</th>
        <th class="itsm">ITSM</th>
        <th class="csm">CSM</th>
        <th class="cafm">CAFM</th>
        <th class="foi">FOI</th>
      </tr>
      <tr>
        <td>Ticket Logging</td>
        <td class="price">-</td>
        <td><i class="fal fa-check-circle"></i></td>
        <td><i class="fal fa-check-circle"></i></td>
        <td><i class="fal fa-check-circle"></i></td>
        <td><i class="fal fa-check-circle"></i></td>
      </tr>
      <tr>
        <td><a href="/features/help-desk-notes-audit-trail/">Ticket Notes and History</a></td>
        <td class="price">-</td>
        <td><i class="fal fa-check-circle"></i></td>
        <td><i class="fal fa-check-circle"></i></td>
        <td><i class="fal fa-check-circle"></i></td>
        <td><i class="fal fa-check-circle"></i></td>
      </tr><tr>
        <td><a href="/features/help-desk-activities/">Ticket Sub-Tasks</a></a></td>
        <td class="price">-</td>
        <td><i class="fal fa-check-circle"></i></td>
        <td><i class="fal fa-check-circle"></i></td>
        <td><i class="fal fa-check-circle"></i></td>
        <td><i class="fal fa-check-circle"></i></td>
      </tr><tr>
        <td><a href="/features/helpdesk-call-template/">Quick Ticket Templates</a></a></td>
        <td class="price">-</td>
        <td><i class="fal fa-check-circle"></i></td>
        <td><i class="fal fa-check-circle"></i></td>
        <td><i class="fal fa-check-circle"></i></td>
        <td><i class="fal fa-check-circle"></i></td>
      </tr>
      <tr>
        <td>Types, Sub-Types, Priorities, Statuses, Analysis etc.</td>
        <td class="price">-</td>
        <td><i class="fal fa-check-circle"></i></td>
        <td><i class="fal fa-check-circle"></i></td>
        <td><i class="fal fa-check-circle"></i></td>
        <td><i class="fal fa-check-circle"></i></td>
      </tr>
      <tr>
        <td><a href="/features-chat-2/">Branded Chat Messaging App</a></a></td>
        <td class="price">£5 per Agent pm</td>
        <td><i class="fal fa-check-circle"></i></td>
        <td><i class="fal fa-check-circle"></i></td>
        <td><i class="fal fa-check-circle"></i></td>
        <td><i class="fal fa-check-circle"></i></td>
      </tr>
      <tr>
        <td><a href="/features/mobile-service-desk/">Customizable Mobile Web App</a></td>
        <td class="price">-</td>
        <td><i class="fal fa-check-circle"></i></td>
        <td><i class="fal fa-check-circle"></i></td>
        <td><i class="fal fa-check-circle"></i></td>
        <td><i class="fal fa-check-circle"></i></td>
      </tr>
      <tr>
        <td>Customer Database</td>
        <td class="price">-</td>
        <td><i class="fal fa-check-circle"></i></td>
        <td><i class="fal fa-check-circle"></i></td>
        <td><i class="fal fa-check-circle"></i></td>
        <td><i class="fal fa-check-circle"></i></td>
      </tr>
      <tr>
        <td>Contact Database</td>
        <td class="price">-</td>
        <td><i class="fal fa-check-circle"></i></td>
        <td><i class="fal fa-check-circle"></i></td>
        <td><i class="fal fa-check-circle"></i></td>
        <td><i class="fal fa-check-circle"></i></td>
      </tr>
      <tr>
        <td>Product Database</td>
        <td class="price">-</td>
        <td><i class="fal fa-check-circle"></i></td>
        <td><i class="fal fa-check-circle"></i></td>
        <td><i class="fal fa-check-circle"></i></td>
        <td><i class="fal fa-check-circle"></i></td>
      </tr>
      <tr>
        <td>Inventory/Asset Database</td>
        <td class="price">-</td>
        <td><i class="fal fa-check-circle"></i></td>
        <td><i class="fal fa-check-circle"></i></td>
        <td><i class="fal fa-check-circle"></i></td>
        <td><i class="fal fa-check-circle"></i></td>
      </tr>
      <tr>
        <td><a href="/features/knowledge-base/">Knowledgebase</a></td>
        <td class="price">-</td>
        <td><i class="fal fa-check-circle"></i></td>
        <td><i class="fal fa-check-circle"></i></td>
        <td><i class="fal fa-check-circle"></i></td>
        <td><i class="fal fa-check-circle"></i></td>
      </tr>
      <tr>
        <td><a href="/features/self-service/">Branded Customer Self-Service Portal</a></td>
        <td class="price">-</td>
        <td><i class="fal fa-check-circle"></i></td>
        <td><i class="fal fa-check-circle"></i></td>
        <td><i class="fal fa-check-circle"></i></td>
        <td><i class="fal fa-check-circle"></i></td>
      </tr>
      <tr>
        <td>Create Custom Views</td>
        <td class="price">-</td>
        <td><i class="fal fa-check-circle"></i></td>
        <td><i class="fal fa-check-circle"></i></td>
        <td><i class="fal fa-check-circle"></i></td>
        <td><i class="fal fa-check-circle"></i></td>
      </tr>
      <tr>
        <td><a href="/features/workflow-engine/">Workflow Designer</a></td>
        <td class="price">-</td>
        <td><i class="fal fa-check-circle"></i></td>
        <td><i class="fal fa-check-circle"></i></td>
        <td><i class="fal fa-check-circle"></i></td>
        <td><i class="fal fa-check-circle"></i></td>
      </tr>
      <tr>
        <td>Attach Documents</td>
        <td class="price">-</td>
        <td><i class="fal fa-check-circle"></i></td>
        <td><i class="fal fa-check-circle"></i></td>
        <td><i class="fal fa-check-circle"></i></td>
        <td><i class="fal fa-check-circle"></i></td>
      </tr>
      <tr>
        <td>Create Unlimited Custom Fields</td>
        <td class="price">-</td>
        <td><i class="fal fa-check-circle"></i></td>
        <td><i class="fal fa-check-circle"></i></td>
        <td><i class="fal fa-check-circle"></i></td>
        <td><i class="fal fa-check-circle"></i></td>
      </tr>
      <tr>
        <td>Mandatory / Readonly Fields</td>
        <td class="price">-</td>
        <td><i class="fal fa-check-circle"></i></td>
        <td><i class="fal fa-check-circle"></i></td>
        <td><i class="fal fa-check-circle"></i></td>
        <td><i class="fal fa-check-circle"></i></td>
      </tr>
      <tr>
        <td><a href="/features/drag-and-drop-dashboard-builder/"> Drag & Drop Form and Dashboard Designer</a></td>
        <td class="price">-</td>
        <td><i class="fal fa-check-circle"></i></td>
        <td><i class="fal fa-check-circle"></i></td>
        <td><i class="fal fa-check-circle"></i></td>
        <td><i class="fal fa-check-circle"></i></td>
      </tr>
      <tr>
        <td><a href="/features/customer-portal-branding/"> Customer Portal Branding</a></td>
        <td class="price">-</td>
        <td><i class="fal fa-check-circle"></i></td>
        <td><i class="fal fa-check-circle"></i></td>
        <td><i class="fal fa-check-circle"></i></td>
        <td><i class="fal fa-check-circle"></i></td>
      </tr>
      <tr>
        <td>Dashboard Noticeboard/News Feed</td>
        <td class="price">-</td>
        <td><i class="fal fa-check-circle"></i></td>
        <td><i class="fal fa-check-circle"></i></td>
        <td><i class="fal fa-check-circle"></i></td>
        <td><i class="fal fa-check-circle"></i></td>
      </tr>
      <tr>
        <td>Theme Designer</td>
        <td class="price">-</td>
        <td><i class="fal fa-check-circle"></i></td>
        <td><i class="fal fa-check-circle"></i></td>
        <td><i class="fal fa-check-circle"></i></td>
        <td><i class="fal fa-check-circle"></i></td>
      </tr>
      <tr>
        <td><a href="/features/help-desk-timeline/"> Timeline</a></td>
        <td class="price">-</td>
        <td><i class="fal fa-check-circle"></i></td>
        <td><i class="fal fa-check-circle"></i></td>
        <td><i class="fal fa-check-circle"></i></td>
        <td><i class="fal fa-check-circle"></i></td>
      </tr>
      <tr>
        <td><a href="/features/kanban-workboard/">Kanban / Workboards</a></td>
        <td class="price">-</td>
        <td><i class="fal fa-check-circle"></i></td>
        <td><i class="fal fa-check-circle"></i></td>
        <td><i class="fal fa-check-circle"></i></td>
        <td><i class="fal fa-check-circle"></i></td>
      </tr>
      <tr>
        <td>Wallboard</td>
        <td class="price">-</td>
        <td><i class="fal fa-check-circle"></i></td>
        <td><i class="fal fa-check-circle"></i></td>
        <td><i class="fal fa-check-circle"></i></td>
        <td><i class="fal fa-check-circle"></i></td>
      </tr>
      <tr>
        <td><a href="/features/ldap-integration/">MS/AD LDAP Connector & Outlook/Azure SSO*</a></td>
        <td class="price">-</td>
        <td><i class="fal fa-check-circle"></i></td>
        <td><i class="fal fa-check-circle"></i></td>
        <td><i class="fal fa-check-circle"></i></td>
        <td><i class="fal fa-check-circle"></i></td>
      </tr>
      <tr>
        <td>SLA Management 3rd Party Monitoring</td>
        <td class="price">-</td>
        <td><i class="fal fa-check-circle"></i></td>
        <td><i class="fal fa-check-circle"></i></td>
        <td><i class="fal fa-check-circle"></i></td>
        <td><i class="fal fa-check-circle"></i></td>
      </tr>
      <tr>
        <td>Customer Satisfaction Feedback Surveys & Emojis</td>
        <td class="price">-</td>
        <td><i class="fal fa-check-circle"></i></td>
        <td><i class="fal fa-check-circle"></i></td>
        <td><i class="fal fa-check-circle"></i></td>
        <td><i class="fal fa-check-circle"></i></td>
      </tr>
      <tr>
        <td>Drag & Drop File Import</td>
        <td class="price">-</td>
        <td><i class="fal fa-check-circle"></i></td>
        <td><i class="fal fa-check-circle"></i></td>
        <td><i class="fal fa-check-circle"></i></td>
        <td><i class="fal fa-check-circle"></i></td>
      </tr>
      <tr>
        <td>Software Support (Telephone, Email, Chat)</td>
        <td class="price">Inc with 5 or more agents or £500 py</td>
        <td><i class="fal fa-check-circle"></i></td>
        <td><i class="fal fa-check-circle"></i></td>
        <td><i class="fal fa-check-circle"></i></td>
        <td><i class="fal fa-check-circle"></i></td>
      </tr>

      <tr>
        <td><a href="/features/microsoft-teams-integration/">Microsoft Teams Integration</a></td>
        <td class="price">£500 py</td>
        <td><i class="fal fa-check-circle"></i></td>
        <td><i class="fal fa-check-circle"></i></td>
        <td><i class="fal fa-check-circle"></i></td>
        <td><i class="fal fa-check-circle"></i></td>
      </tr>
      <tr>
        <td>Visitor Management </td>        
        <td class="price">£500 py</td>
        <td>Add-On</td>
        <td>Add-On</td>
        <td>Add-On</td>
        <td>Add-On</td>
      </tr>
      <tr>
        <td><a href="/features/resource-booking-software/">Resource / Amenity Booking</a></td>        
        <td class="price">£500 py</td>
        <td>Add-On</td>
        <td>Add-On</td>
        <td>Add-On</td>
        <td>Add-On</td>
      </tr>

      
      <tr type="button" id="button" class="category-container" onclick="dropdown('platformservice')">
        <th class="category">Platform Service <i class="fas fa-chevron-right"></i><i class="fas fa-chevron-down"></i></th>
        <th class="category"></th>
        <th class="category"></th>
        <th class="category"></th>
        <th class="category"></th>
        <th class="category"></th>
      </tr>
      <tr class="collapsible platformservice" style="">
        <td>Opt-in-Opt out Subscription</td>
        <td class="price">-</td>
        <td><i class="fal fa-times-circle"></i></td>
        <td><i class="fal fa-times-circle"></i></td>
        <td><i class="fal fa-times-circle"></i></td>
        <td><i class="fal fa-times-circle"></i></td>
      </tr>  
      <tr class="collapsible platformservice" style="">
        <td>Microsoft Azure Platform</td>
        <td class="price">-</td>
        <td><i class="fal fa-check-circle"></i></td>
        <td><i class="fal fa-check-circle"></i></td>
        <td><i class="fal fa-check-circle"></i></td>
        <td><i class="fal fa-check-circle"></i></td>
      </tr>
      <tr class="collapsible platformservice" style="">
        <td>Daily Backup (x3)</td>
        <td class="price">-</td>
        <td><i class="fal fa-check-circle"></i></td>
        <td><i class="fal fa-check-circle"></i></td>
        <td><i class="fal fa-check-circle"></i></td>
        <td><i class="fal fa-check-circle"></i></td>
      </tr>
      <tr type="button"  class="category-container"  onclick="dropdown('softwareassetmanagement')">
        <th class="category"><a href="/features/asset-and-software-licence-management/"> Software Asset Management <i class="fas fa-chevron-right"></i><i class="fas fa-chevron-down"></i></a></th>
        <th class="category">£1000 py</th>
        <th class="category"></th>
        <th class="category"></th>
        <th class="category"></th>
        <th class="category"></th>
      </tr>
      <tr class="collapsible softwareassetmanagement" style="">
        <td>PC/Server Software & Hardware Auditing*</td>
        <td class="price">-</td>
        <td>Add-On</td>
        <td>Add-On</td>
        <td>Add-On</td>
        <td>Add-On</td>
      </tr>
      <tr class="collapsible softwareassetmanagement" style="">
        <td>SNMP Discovery*</td>
        <td class="price">-</td>
        <td>Add-On</td>
        <td>Add-On</td>
        <td>Add-On</td>
        <td>Add-On</td>
      </tr>
      <tr class="collapsible softwareassetmanagement" style="">
        <td><a href="/features/asset-and-software-licence-management/">Software Licence Manager*</a></td>
        <td class="price">-</td>
        <td>Add-On</td>
        <td>Add-On</td>
        <td>Add-On</td>
        <td>Add-On</td>
      </tr>
      <tr type="button"  class="category-container"  onclick="dropdown('email')">
        <th class="category"><a href="/features/email-integration/">Email <i class="fas fa-chevron-right"></i><i class="fas fa-chevron-down"></i></a></th>
        <th class="category"></th>
        <th class="category"></th>
        <th class="category"></th>
        <th class="category"></th>
        <th class="category"></th>
      </tr>
      <tr class="collapsible email" style="">
        <td>Outbound Office365, Gmail,....</td>
        <td class="price">-</td>
        <td><i class="fal fa-check-circle"></i></td>
        <td><i class="fal fa-check-circle"></i></td>
        <td><i class="fal fa-check-circle"></i></td>
        <td><i class="fal fa-check-circle"></i></td>
      </tr>
      <tr class="collapsible email" style="">
        <td>Inbound creation of Tickets from Emails*</td>
        <td class="price">-</td>
        <td><i class="fal fa-check-circle"></i></td>
        <td><i class="fal fa-check-circle"></i></td>
        <td><i class="fal fa-check-circle"></i></td>
        <td><i class="fal fa-check-circle"></i></td>
      </tr>
      <tr class="collapsible email" style="">
        <td>Outlook 365 Add-In</td>
        <td class="price">-</td>
        <td><i class="fal fa-check-circle"></i></td>
        <td><i class="fal fa-check-circle"></i></td>
        <td><i class="fal fa-check-circle"></i></td>
        <td><i class="fal fa-check-circle"></i></td>
      </tr>
      <tr class="collapsible email" style="">
        <td>Full Email / Letter Template Designer</td>
        <td class="price">-</td>
        <td><i class="fal fa-check-circle"></i></td>
        <td><i class="fal fa-check-circle"></i></td>
        <td><i class="fal fa-check-circle"></i></td>
        <td><i class="fal fa-check-circle"></i></td>
      </tr>
      <tr type="button"  class="category-container"  onclick="dropdown('reports')">
        <th class="category"><a href="/features/reports-2/"> Reports <i class="fas fa-chevron-right"></i><i class="fas fa-chevron-down"></i></a></th>
        <th class="category"></th>
        <th class="category"></th>
        <th class="category"></th>
        <th class="category"></th>
        <th class="category"></th>
      </tr>
      <tr class="collapsible reports" style="">
        <td>Default set of prebuilt Reports</td>
        <td class="price">-</td>
        <td><i class="fal fa-check-circle"></i></td>
        <td><i class="fal fa-check-circle"></i></td>
        <td><i class="fal fa-check-circle"></i></td>
        <td><i class="fal fa-check-circle"></i></td>
      </tr>
      <tr class="collapsible reports" style="">
        <td>Create Reports & Add to Dashboards</td>
        <td class="price">-</td>
        <td><i class="fal fa-check-circle"></i></td>
        <td><i class="fal fa-check-circle"></i></td>
        <td><i class="fal fa-check-circle"></i></td>
        <td><i class="fal fa-check-circle"></i></td>
      </tr>
      <tr class="collapsible reports" style="">
        <td>Schedule pdf/csv Reports via Email</td>
        <td class="price">-</td>
        <td><i class="fal fa-check-circle"></i></td>
        <td><i class="fal fa-check-circle"></i></td>
        <td><i class="fal fa-check-circle"></i></td>
        <td><i class="fal fa-check-circle"></i></td>
      </tr>
      <tr class="collapsible reports" style="">
        <td>RAG Reporting</td>
        <td class="price">-</td>
        <td><i class="fal fa-check-circle"></i></td>
        <td><i class="fal fa-check-circle"></i></td>
        <td><i class="fal fa-check-circle"></i></td>
        <td><i class="fal fa-check-circle"></i></td>
      </tr>
      <tr class="collapsible reports" style="">
        <td>Drilldown</td>
        <td class="price">-</td>
        <td><i class="fal fa-check-circle"></i></td>
        <td><i class="fal fa-check-circle"></i></td>
        <td><i class="fal fa-check-circle"></i></td>
        <td><i class="fal fa-check-circle"></i></td>
      </tr>
      <tr class="collapsible reports" style="">
        <td>Report on any data in SupportDesk</td>
        <td class="price">-</td>
        <td><i class="fal fa-check-circle"></i></td>
        <td><i class="fal fa-check-circle"></i></td>
        <td><i class="fal fa-check-circle"></i></td>
        <td><i class="fal fa-check-circle"></i></td>
      </tr>
      <tr class="collapsible reports" style="">
        <td><a href="supportdesk/features/kpi-monitor/">KPI Monitor with Autorefresh</td>
        <td class="price">-</td>
        <td><i class="fal fa-check-circle"></i></td>
        <td><i class="fal fa-check-circle"></i></td>
        <td><i class="fal fa-check-circle"></i></td>
        <td><i class="fal fa-check-circle"></i></td>
      </tr>
      <tr type="button"  class="category-container"  onclick="dropdown('itsmconfiguration')">
        <th class="category">ITSM Configuration <i class="fas fa-chevron-right"></i><i class="fas fa-chevron-down"></i></th>
        <th class="category">£1,500 py</th>
        <th class="category"><i class="fal fa-check-circle"></i></th>
        <th class="category">Add-On</th>
        <th class="category">Add-On</th>
        <th class="category">Add-On</th>
      </tr>
      <tr class="collapsible itsmconfiguration" style="">
        <td>Incident Management</td>
        <td class="price">-</td>
        <td><i class="fal fa-check-circle"></i></td>
        <td>Add-On</td>
        <td>Add-On</td>
        <td>Add-On</td>
      </tr>
      <tr class="collapsible itsmconfiguration" style="">
        <td>Problem Management</td>
        <td class="price">-</td>
        <td><i class="fal fa-check-circle"></i></td>
        <td>Add-On</td>
        <td>Add-On</td>
        <td>Add-On</td>
      </tr>
      <tr class="collapsible itsmconfiguration" style="">
        <td>Change Management</td>
        <td class="price">-</td>
        <td><i class="fal fa-check-circle"></i></td>
        <td>Add-On</td>
        <td>Add-On</td>
        <td>Add-On</td>
      </tr>
      <tr class="collapsible itsmconfiguration" style="">
        <td>Service Request Management</td>
        <td class="price">-</td>
        <td><i class="fal fa-check-circle"></i></td>
        <td>Add-On</td>
        <td>Add-On</td>
        <td>Add-On</td>
      </tr>
      <tr class="collapsible itsmconfiguration" style="">
        <td>Release Management</td>
        <td class="price">-</td>
        <td><i class="fal fa-check-circle"></i></td>
        <td>Add-On</td>
        <td>Add-On</td>
        <td>Add-On</td>
      </tr>
      <tr type="button"  class="category-container"  onclick="dropdown('enterprisemds')">
        <th class="category">Enterprise Multi-Dept ServiceDesk <i class="fas fa-chevron-right"></i><i class="fas fa-chevron-down"></i></th>
        <th class="category">£1,500 py</th>
        <th class="category">Add-On</th>
        <th class="category">Add-On</th>
        <th class="category">Add-On</th>
        <th class="category">Add-On</th>
      </tr>
      <tr class="collapsible enterprisemds" style="">
        <td>CAFM Service Desk</td>
        <td class="price">-</td>
        <td>Add-On</td>
        <td>Add-On</td>
        <td><i class="fal fa-check-circle"></i></td>
        <td>Add-On</td>
      </tr>
      <tr class="collapsible enterprisemds" style="">
        <td>HR Service Desk</td>
        <td class="price">-</td>
        <td>Add-On</td>
        <td>Add-On</td>
        <td>Add-On</td>
        <td>Add-On</td>
      </tr>
      <tr class="collapsible enterprisemds" style="">
        <td>Finance Service Desk</td>
        <td class="price">-</td>
        <td>Add-On</td>
        <td>Add-On</td>
        <td>Add-On</td>
        <td>Add-On</td>
      </tr>
      <tr type="button"  class="category-container"  onclick="dropdown('cafm')">
        <th class="category">CAFM <i class="fas fa-chevron-right"></i><i class="fas fa-chevron-down"></i></th>
        <th class="category">£750 py</th>
        <th class="category">Add-On</th>
        <th class="category">Add-On</th>
        <th class="category"></th>
        <th class="category">Add-On</th>
      </tr>
      <tr class="collapsible cafm" style="">
        <td>Planned Preventative Management</td>
        <td class="price">-</td>
        <td>Add-On</td>
        <td>Add-On</td>
        <td><i class="fal fa-check-circle"></i></td>
        <td>Add-On</td>
      </tr>
      <tr class="collapsible cafm" style="">
        <td>Reactive Job Logging</td>
        <td class="price">-</td>
        <td>Add-On</td>
        <td>Add-On</td>
        <td><i class="fal fa-check-circle"></i></td>
        <td>Add-On</td>
      </tr>
      <tr class="collapsible cafm" style="">
        <td>Mobile Signature Job Sign-Off</td>
        <td class="price">-</td>
        <td>Add-On</td>
        <td>Add-On</td>
        <td><i class="fal fa-check-circle"></i></td>
        <td>Add-On</td>
      </tr>
      <tr class="collapsible cafm" style="">
        <td>Engineer Job Notification</td>
        <td class="price">-</td>
        <td>Add-On</td>
        <td>Add-On</td>
        <td><i class="fal fa-check-circle"></i></td>
        <td>Add-On</td>
      </tr>
      <tr class="collapsible cafm" style="">
        <td><a href="/features/engineers-location-map/">Engineer Location Map</a></td>
        <td class="price">-</td>
        <td>Add-On</td>
        <td>Add-On</td>
        <td><i class="fal fa-check-circle"></i></td>
        <td>Add-On</td>
      </tr>
      <tr class="collapsible cafm" style="">
        <td>Unlimited Engineer/Contractor 3rd Party Access Portal (Desktop & Mobile)</td>
        <td class="price">£500 py</td>
        <td>Add-On</td>
        <td>Add-On</td>
        <td>Add-On</td>
        <td>Add-On</td>
      </tr>
      <tr type="button"  class="category-container"  onclick="dropdown('facilitiesmanagement')">
        <th class="category"><a href="/features/financial-management/">Finance <i class="fas fa-chevron-right"></i><i class="fas fa-chevron-down"></i></a></th>
        <th class="category">£500 py</th>
        <th class="category">Add-On</th>
        <th class="category">Add-On</th>
        <th class="category"><i class="fal fa-check-circle"></i></th>
        <th class="category">Add-On</th>
      </tr>
      <tr class="collapsible facilitiesmanagement" style="">
        <td>Suppliers</td>
        <td class="price">-</td>
        <td>Add-On</td>
        <td>Add-On</td>
        <td><i class="fal fa-check-circle"></i></td>
        <td>Add-On</td>
      </tr>
      <tr class="collapsible facilitiesmanagement" style="">
        <td>Contract Management</td>
        <td class="price">-</td>
        <td>Add-On</td>
        <td>Add-On</td>
        <td><i class="fal fa-check-circle"></i></td>
        <td>Add-On</td>
      </tr>
      <tr class="collapsible facilitiesmanagement" style="">
        <td>Purchase Orders</td>
        <td class="price">-</td>
        <td>Add-On</td>
        <td>Add-On</td>
        <td><i class="fal fa-check-circle"></i></td>
        <td>Add-On</td>
      </tr>
      <tr class="collapsible facilitiesmanagement" style="">
        <td>Quotes</td>
        <td class="price">-</td>
        <td>Add-On</td>
        <td>Add-On</td>
        <td><i class="fal fa-check-circle"></i></td>
        <td>Add-On</td>
      </tr>

      <tr type="button"  class="category-container"  onclick="dropdown('crm')">
        <th class="category"><a href="/features/customer-relationship-management/">CRM <i class="fas fa-chevron-right"></i><i class="fas fa-chevron-down"></i></a></th>
        <th class="category">£750 py</th>
        <th class="category">Add-On</th>
        <th class="category"></th>
        <th class="category">Add-On</th>
        <th class="category">Add-On</th>
      </tr>
      <tr class="collapsible crm" style="">
        <td>Contact Profiling</td>
        <td class="price">-</td>
        <td>Add-On</td>
        <td><i class="fal fa-check-circle"></i></td>
        <td><i class="fal fa-check-circle"></i></td>
        <td><i class="fal fa-check-circle"></i></td>
      </tr>
      <tr class="collapsible crm" style="">
        <td>Marketing Campaigns</td>
        <td class="price">-</td>
        <td>Add-On</td>
        <td><i class="fal fa-check-circle"></i></td>
        <td><i class="fal fa-check-circle"></i></td>
        <td><i class="fal fa-check-circle"></i></td>
      </tr>
      <tr class="collapsible crm" style="">
        <td>Opportunity Management</td>
        <td class="price">-</td>
        <td>Add-On</td>
        <td><i class="fal fa-check-circle"></i></td>
        <td><i class="fal fa-check-circle"></i></td>
        <td><i class="fal fa-check-circle"></i></td>
      </tr>
      <tr type="button"  class="category-container"  onclick="dropdown('case')">
        <th class="category">Case,Complaints & FOI Management <i class="fas fa-chevron-right"></i><i class="fas fa-chevron-down"></i></th>
        <th class="category">£750 py</th>
        <th class="category">Add-On</th>
        <th class="category">Add-On</th>
        <th class="category">Add-On</th>
        <th class="category"><i class="fal fa-check-circle"></i></th>
      </tr>
      <tr class="collapsible case" style="">
        <td>Public Facing Form Submissing </td>
        <td class="price">-</td>
        <td>Add-On</td>
        <td>Add-On</td>
        <td>Add-On</td>
        <td><i class="fal fa-check-circle"></i></td>
      </tr>
      <tr class="collapsible case" style="">
        <td>Access from customer website </td>
        <td class="price">-</td>
        <td>Add-On</td>
        <td>Add-On</td>
        <td>Add-On</td>
        <td><i class="fal fa-check-circle"></i></td>
      </tr>
      <tr class="collapsible case" style="">
        <td>Paperless Progression</td>
        <td class="price">-</td>
        <td>Add-On</td>
        <td>Add-On</td>
        <td>Add-On</td>
        <td><i class="fal fa-check-circle"></i></td>
      </tr>
      <tr class="collapsible case" style="">
        <td>Deadline Alerts</td>
        <td class="price">-</td>
        <td>Add-On</td>
        <td>Add-On</td>
        <td>Add-On</td>
        <td><i class="fal fa-check-circle"></i></td>
      </tr>
      <tr class="collapsible case" style="">
        <td>Full Disclosure Log</td>
        <td class="price">-</td>
        <td>Add-On</td>
        <td>Add-On</td>
        <td>Add-On</td>
        <td><i class="fal fa-check-circle"></i></td>
      </tr>
      <tr class="collapsible case" style="">
        <td>Request Delegation</td>
        <td class="price">-</td>
        <td>Add-On</td>
        <td>Add-On</td>
        <td>Add-On</td>
        <td><i class="fal fa-check-circle"></i></td>
      </tr>
      <tr class="collapsible case" style="">
        <td>Case Reports</td>
        <td class="price">-</td>
        <td>Add-On</td>
        <td>Add-On</td>
        <td>Add-On</td>
        <td><i class="fal fa-check-circle"></i></td>
      </tr>
      <tr class="collapsible case" style="">
        <td>Pre-Built Letter Templates </td>
        <td class="price">-</td>
        <td>Add-On</td>
        <td>Add-On</td>
        <td>Add-On</td>
        <td><i class="fal fa-check-circle"></i></td>
      </tr>
      <tr class="collapsible case" style="">
        <td>Information Governance </td>
        <td class="price">£500 py</td>
        <td>Add-On</td>
        <td>Add-On</td>
        <td>Add-On</td>
        <td>Add-On</td>
      </tr>
      <tr class="collapsible case" style="">
        <td>Record of Processing Activities (RoPA)</td>
        <td class="price">£500 py</td>
        <td>Add-On</td>
        <td>Add-On</td>
        <td>Add-On</td>
        <td>Add-On</td>
      </tr>
      <tr class="collapsible case" style="">
        <td>Unlimited Departmental Data Requesters</td>
        <td class="price">£750 py</td>
        <td>Add-On</td>
        <td>Add-On</td>
        <td>Add-On</td>
        <td>Add-On</td>
      </tr>
      <tr type="button"  class="category-container"  onclick="dropdown('api')">
        <th class="category">Integrations, API's and Connectors <i class="fas fa-chevron-right"></i><i class="fas fa-chevron-down"></i></th>
        <th class="category"></th>
        <th class="category"></th>
        <th class="category"></th>
        <th class="category"></th>
        <th class="category"></th>
      </tr>
      <tr class="collapsible api" style="">
        <td>SCCM, Lansweeper, Certero, Snow, Altiris Connectors</td>
        <td class="price">£500 py</td>
        <td>Add-On</td>
        <td>Add-On</td>
        <td>Add-On</td>
        <td>Add-On</td>
      </tr>
      <tr class="collapsible api" style="">
        <td><a href="/features/microsoft-teams/">Microsoft Teams Ticket Creation</a></td>
        <td class="price">-</td>
        <td><i class="fal fa-check-circle"></i></td>
        <td><i class="fal fa-check-circle"></i></td>
        <td><i class="fal fa-check-circle"></i></td>
        <td><i class="fal fa-check-circle"></i></td>
      </tr>
      <tr class="collapsible api" style="">
        <td><a href="supportdesk/features/help-desk-twitter-integration/">Twitter</a></td>
        <td class="price">-</td>
        <td><i class="fal fa-check-circle"></i></td>
        <td><i class="fal fa-check-circle"></i></td>
        <td><i class="fal fa-check-circle"></i></td>
        <td><i class="fal fa-check-circle"></i></td>
      </tr> 
      <tr class="collapsible api" style="">
        <td><a href="/features/jira-integration/">Jira</a></td>
        <td class="price">-</td>
        <td><i class="fal fa-check-circle"></i></td>
        <td><i class="fal fa-check-circle"></i></td>
        <td><i class="fal fa-check-circle"></i></td>
        <td><i class="fal fa-check-circle"></i></td>
      </tr>
      <tr class="collapsible api" style="">
        <td>CSV Import / Export</td>
        <td class="price">-</td>
        <td><i class="fal fa-check-circle"></i></td>
        <td><i class="fal fa-check-circle"></i></td>
        <td><i class="fal fa-check-circle"></i></td>
        <td><i class="fal fa-check-circle"></i></td>
      </tr> 
      <tr class="collapsible api" style="">
        <td>Transfer of Legacy Data using CSV</td>
        <td class="price">-</td>
        <td><i class="fal fa-check-circle"></i></td>
        <td><i class="fal fa-check-circle"></i></td>
        <td><i class="fal fa-check-circle"></i></td>
        <td><i class="fal fa-check-circle"></i></td>
      </tr>
      <tr class="collapsible api" style="">
        <td>Slack Ticket Creation</td>
        <td class="price">-</td>
        <td><i class="fal fa-check-circle"></i></td>
        <td><i class="fal fa-check-circle"></i></td>
        <td><i class="fal fa-check-circle"></i></td>
        <td><i class="fal fa-check-circle"></i></td>
      </tr>
      <tr class="collapsible api" style="">
        <td>Xero Integration</td>
        <td class="price">-</td>
        <td><i class="fal fa-check-circle"></i></td>
        <td><i class="fal fa-check-circle"></i></td>
        <td><i class="fal fa-check-circle"></i></td>
        <td><i class="fal fa-check-circle"></i></td>
      </tr>
      <tr class="collapsible api" style="">
        <td>Report JSON Api</td>
        <td class="price">-</td>
        <td><i class="fal fa-check-circle"></i></td>
        <td><i class="fal fa-check-circle"></i></td>
        <td><i class="fal fa-check-circle"></i></td>
        <td><i class="fal fa-check-circle"></i></td>
      </tr>
      <tr class="collapsible api" style="">
        <td><a href="/features/remote-session-request-button/">Bomgar Remote Session Launch Button</a></td>
        <td class="price">-</td>
        <td><i class="fal fa-check-circle"></i></td>
        <td><i class="fal fa-check-circle"></i></td>
        <td><i class="fal fa-check-circle"></i></td>
        <td><i class="fal fa-check-circle"></i></td>
      </tr>
      <tr class="collapsible api" style="">
        <td><a href="/features/skype-for-business-integration/">Skype for Business</a></td>
        <td class="price">-</td>
        <td><i class="fal fa-check-circle"></i></td>
        <td><i class="fal fa-check-circle"></i></td>
        <td><i class="fal fa-check-circle"></i></td>
        <td><i class="fal fa-check-circle"></i></td>
      </tr>
      <tr type="button"  class="category-container"  onclick="dropdown('advancedfeatures')">
        <th class="category">Advanced Features <i class="fas fa-chevron-right"></i><i class="fas fa-chevron-down"></i></th>
        <th class="category"></th>
        <th class="category"></th>
        <th class="category"></th>
        <th class="category"></th>
        <th class="category"></th>
      </tr>
      <tr class="collapsible advancedfeatures" style="">
        <td>Document Management</td>
        <td class="price">£750 py</td>
        <td>Add-On</td>
        <td>Add-On</td>
        <td>Add-On</td>
        <td>Add-On</td>
      </tr>
      <tr class="collapsible advancedfeatures" style="">
        <td>Multi-Domain Support</td>
        <td class="price">£500 py</td>
        <td>Add-On</td>
        <td>Add-On</td>
        <td>Add-On</td>
        <td>Add-On</td>
      </tr>
      <tr class="collapsible advancedfeatures" style="">
        <td><a href="/features/telephony-integration/">Telephony Agent</a></td>
        <td class="price">£500 py</td>
        <td>Add-On</td>
        <td>Add-On</td>
        <td>Add-On</td>
        <td>Add-On</td>
      </tr>
      <tr class="collapsible advancedfeatures" style="">
        <td>Powershell Integration</td>
        <td class="price">£500 py</td>
        <td>Add-On</td>
        <td>Add-On</td>
        <td>Add-On</td>
        <td>Add-On</td>
      </tr>
      <tr class="collapsible advancedfeatures" style="">
        <td>IP Whitelisting*</td>
        <td class="price">-</td>
        <td><i class="fal fa-check-circle"></i></td>
        <td><i class="fal fa-check-circle"></i></td>
        <td><i class="fal fa-check-circle"></i></td>
        <td><i class="fal fa-check-circle"></i></td>
      </tr>
      <tr class="collapsible advancedfeatures" style="">
        <td>Sandbox Environment</td>
        <td class="price">£500 py</td>
        <td>Add-On</td>
        <td>Add-On</td>
        <td>Add-On</td>
        <td>Add-On</td>
      </tr>
      <tr class="collapsible advancedfeatures" style="">
        <td><a href="/features/multi-lingual-help-desk-support/">Multilingual Support</a></td>
        <td class="price">£500 py</td>
        <td>Add-On</td>
        <td>Add-On</td>
        <td>Add-On</td>
        <td>Add-On</td>
      </tr>
      <tr class="collapsible advancedfeatures" style="">
        <td>Onsite Synchronised Backup*</td>
        <td class="price">£1,500 py</td>
        <td>Add-On</td>
        <td>Add-On</td>
        <td>Add-On</td>
        <td>Add-On</td>
      </tr>
      <tr class="collapsible advancedfeatures" style="">
        <td>Dark Mode </td>
        <td class="price">-</td>
        <td>Add-On</td>
        <td>Add-On</td>
        <td>Add-On</td>
        <td>Add-On</td>
      </tr>
      <tr class="collapsible advancedfeatures" style="">
        <td>Advanced UI Developer</td>
        <td class="price">-</td>
        <td><i class="fal fa-check-circle"></i></td>
        <td><i class="fal fa-check-circle"></i></td>
        <td><i class="fal fa-check-circle"></i></td>
        <td><i class="fal fa-check-circle"></i></td>
      </tr> 
      <tr class="collapsible advancedfeatures" style="">
        <td>Custom Keyboard Shortcuts </td>
        <td class="price">-</td>
        <td><i class="fal fa-check-circle"></i></td>
        <td><i class="fal fa-check-circle"></i></td>
        <td><i class="fal fa-check-circle"></i></td>
        <td><i class="fal fa-check-circle"></i></td>
      </tr>      
      <tr class="pricing-key">
        <td>
          <p>* Requires setup <br> ** Requires consultancy</p>
        </td>
      </tr>
    </tbody>
  </table>
</div>

<script>
function dropdown(classname) {
    var collapsible = document.querySelectorAll(".collapsible." + classname);
    var arr = Array.from(collapsible);
    arr.forEach(function (item) {
        if (item.style.display === 'table-row') {
            item.style.display = '';
            event.target.parentNode.classList.remove("active");
        }
        else {
            item.style.display = 'table-row';
            event.target.parentNode.classList.add("active");
        }
    });
}


var acc = document.getElementsByClassName("accordion");
var i;

for (i = 0; i < acc.length; i++) {
  acc[i].addEventListener("click", function() {
    this.classList.toggle("active");
    var panel = this.nextElementSibling;
    if (panel.style.display === "block") {
      panel.style.display = "none";
    } else {
      panel.style.display = "block";
    }
  });
}



function openfeature(evt, cityName) {
  var i, featuretabcontent, tablinks;
  featuretabcontent = document.getElementsByClassName("featuretabcontent");
  for (i = 0; i < featuretabcontent.length; i++) {
    featuretabcontent[i].style.display = "none";
  }
  tablinks = document.getElementsByClassName("tablinks");
  for (i = 0; i < tablinks.length; i++) {
    tablinks[i].className = tablinks[i].className.replace(" active", "");
  }
  document.getElementById(cityName).style.display = "block";
  evt.currentTarget.className += " active";
}
</script>
<?php get_footer();?>